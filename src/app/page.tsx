...
export default function Dashboard() {
  ...
  const handleCreateAgent = useCallback(async () => {
    ...
    try {
      ...
      await setAdapterConfig(created.id, adapterConfig);
        ...
        <div className="flex items-center gap-5">
          <div className="flex items-center gap-2">
            <img
              ...
              alt="Kanbun"
              style={{ width: 24, height: 24, objectFit: "cover", borderRadius: 2, transition: "opacity 0.3s" }}
            />
        ...
        <main className="flex-1 flex flex-col overflow-hidden" style={{ background: "var(--bg)" }}>
          {activeView === "dashboard" && (
            <>
              ...
              />
              <section className="flex-1 overflow-y-auto grid-bg" style={{ padding: 16, position: "relative" }}>
                {loadingDashboard && isTauri && (
                  <p className="mn" style={{ marginBottom: 10, fontSize: 10, color: "var(--dim)" }}>
                    Loading dashboard...
                  </p>
                )}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                  {dashboard.projects.length > 0 ? (
                    dashboard.projects.map((projectGroup) => (
                      <div key={projectGroup.project.id}>
                        <div
                          ...
                          style={{ borderBottom: "1px solid var(--border)", paddingBottom: 5, marginBottom: 8 }}
                        >
                          <div className="flex items-center gap-1.5">
                            <div style={{ width: 7, height: 7, background: projectGroup.project.color }} />
                            <span className="hdr" style={{ color: "var(--hi)" }}>
                              {projectGroup.project.name}
                            </span>
                          </div>
                          <span className="mn" style={{ fontSize: 9, color: "var(--dim)" }}>
                            {projectGroup.agents.length} AGENTS
                          </span>
                        </div>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                          {projectGroup.agents.map((agentSummary) => (
                            <AgentCard
                              key={agentSummary.agent.id}
                              summary={agentSummary}
                              selected={selectedAgentId === agentSummary.agent.id}
                              onClick={() => setSelectedAgentId(agentSummary.agent.id)}
                            />
                          ))}
                        </div>
                      </div>
                    ...
                    <>
                      {[0, 1].map((column) => (
                        <div key={`empty-dashboard-col-${column}`} className="empty-dashboard-column" aria-hidden>
                          <div className="empty-dashboard-heading" />
                          <div className="empty-dashboard-cards">
                            {[0, 1, 2, 3].map((card) => (
                              <div key={`empty-dashboard-card-${column}-${card}`} className="empty-dashboard-card">
                                <span className="empty-dashboard-line long" />
                                <span className="empty-dashboard-line medium" />
                                <span className="empty-dashboard-line short" />
                              </div>
                            ))}
                          </div>
                        </div>
                ...
                {showObscuredPane && (
                  <div className="empty-obscure-pane">
                    <div className="empty-obscure-card empty-state-shell">
                      <div className="empty-state-grid">
                        <div className="empty-state-copy">
                          <p className="mn empty-state-kicker">
                            {isTauri ? "WORKSPACE_EMPTY" : "DESKTOP_RUNTIME_REQUIRED"}
                          </p>
                          <p className="empty-state-title">
                            {isTauri
                              ? "No active projects or agents yet."
                              : "This dashboard is intentionally obscured in browser preview."}
                          </p>
                          <p className="mn empty-state-subtitle">
                            {isTauri
                              ? "Set up connectors, then create projects and agents to see live activity here."
                              : "Launch the desktop runtime on port 3002 to view real local state."}
                          </p>
                            ...
                            {isTauri ? (
                              <>
                                <button
                                  ...
                                  disabled={starterBusy || projectBusy || agentBusy}
                                  onClick={() => void handleCreateStarterWorkspace()}
                                >
                                  {starterBusy ? "Creating workspace..." : "Create starter workspace"}
                                </button>
                                <button className="btn-cortex" onClick={() => setActiveView("connectors")}>
                                  Open connectors
                                </button>
                              ...
                              <span className="mn empty-state-command">npm run tauri dev</span>
                            )}
                          </div>
                        </div>

                        <div className="empty-state-ghost" aria-hidden>
                          <div className="empty-state-ghost-header">
                            <span className="empty-state-dot" />
                            <span className="empty-state-dot" />
                            <span className="empty-state-dot" />
                          </div>
                          <div className="empty-state-ghost-columns">
                            <div className="empty-state-ghost-column">
                              <span className="empty-state-ghost-line short" />
                              <span className="empty-state-ghost-line medium" />
                              <span className="empty-state-ghost-line short" />
                              <span className="empty-state-ghost-line long" />
                            </div>
                            <div className="empty-state-ghost-column">
                              <span className="empty-state-ghost-line medium" />
                              <span className="empty-state-ghost-line short" />
